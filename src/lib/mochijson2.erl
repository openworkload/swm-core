...
-define(Q, $").
-define(ADV_COL(S, N), S#decoder{offset = N + S#decoder.offset, column = N + S#decoder.column}).
-define(INC_COL(S), S#decoder{offset = 1 + S#decoder.offset, column = 1 + S#decoder.column}).
-define(INC_LINE(S),
...
json_encode([{K, _} | _] = Props, State) when K =/= struct andalso K =/= array andalso K =/= json ->
...
tokenize_string_fast(B, O) ->
    case B of
        ...
        <<_:O/binary, C1, C2, C3, _/binary>> when C1 >= 224, C1 =< 239, C2 >= 128, C2 =< 191, C3 >= 128, C3 =< 191 ->
            tokenize_string_fast(B, 3 + O);
        <<_:O/binary, C1, C2, C3, C4, _/binary>>
            when C1 >= 240, C1 =< 244, C2 >= 128, C2 =< 191, C3 >= 128, C3 =< 191, C4 >= 128, C4 =< 191 ->
    ...
    case B of
        ...
        <<_:O/binary, "\\u", C3, C2, C1, C0, Rest/binary>> ->
            ...
            if C > 16#D7FF, C < 16#DC00 ->
                   ...
                   [CodePoint] = xmerl_ucs:from_utf16be(<<C:16/big-unsigned-integer, D:16/big-unsigned-integer>>),
                   Acc1 =
                       lists:reverse(
...
e2j_test_vec(utf8) ->
    [{1, "1"},
     ...
     {obj_from_list([{<<"foo">>, []}, {<<"bar">>, obj_from_list([{<<"baz">>, true}])}, {<<"alice">>, <<"bob">>}]),
...
encoder_utf8_test() ->
    ...
    [34, "\\u0001", "\\u0442", "\\u0435", "\\u0441", "\\u0442", 34] = encode(<<1, "Ñ\202ÐµÑ\201Ñ\202">>),
    %% raw utf8 output (optional)
    Enc = mochijson2:encoder([{utf8, true}]),
    [34, "\\u0001", [209, 130], [208, 181], [209, 129], [209, 130], 34] = Enc(<<1, "Ñ\202ÐµÑ\201Ñ\202">>).

input_validation_test() ->
    Good =
        [{16#00A3,
          <<?Q, 16#C2, 16#A3, ?Q>>}, %% pound
...
inline_json_test() ->
    ?assertEqual(<<"\"iodata iodata\"">>, iolist_to_binary(encode({json, [<<"\"iodata">>, " iodata\""]}))),
...
atom_test() ->
    ...
    ?assertEqual(<<"\"\\ud834\\udd20\"">>, iolist_to_binary(encode(list_to_atom(xmerl_ucs:to_utf8(16#0001d120))))),
    ok.
...
key_encode_test() ->
    ...
    ?assertEqual(<<"{\"foo\":1}">>, iolist_to_binary(encode([{<<"foo">>, 1}]))),
    ?assertEqual(<<"{\"foo\":1}">>, iolist_to_binary(encode([{"foo", 1}]))),
    ?assertEqual(<<"{\"\\ud834\\udd20\":1}">>, iolist_to_binary(encode({struct, [{[16#0001d120], 1}]}))),
...
encode_test_() ->
    ...
    [{atom_to_list(F), ?_assertEqual(JSON, iolist_to_binary(encode(decode(JSON, [{format, F}]))))}
