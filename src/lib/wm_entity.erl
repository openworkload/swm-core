...
get_names(local) -> [job];
get_names(local_bag) -> [subscriber, timetable, relocation];
get_names(non_replicable) -> [schema, subscriber, timetable, job];
get_names(with_ids) ->
  [malfunction, hook, grid, cluster, partition, node, service, role, job, project, group, user, queue, scheduler, relocation, image, credential, remote, account, test].
