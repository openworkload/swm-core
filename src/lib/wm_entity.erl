...
-spec extract_json_field_info({binary(), map() | {struct, list()}}) -> {atom(), term(), atom()}.
extract_json_field_info({NameBin, {struct, Props}}) when is_list(Props) ->
    extract_json_field_info({NameBin, maps:from_list(Props)});
extract_json_field_info({NameBin, Field}) when is_map(Field) ->
    TypeBin = maps:get(<<"type">>, Field),
    DefaultBin = maps:get(<<"default">>, Field, []),
