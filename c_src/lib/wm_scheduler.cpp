...
SwmScheduler::SwmScheduler(const char* buf, int* index) {
  if (!buf) {
    std::cerr << "Cannot convert ei buffer into SwmScheduler: null" << std::endl;
    return;
  }
  int term_size = 0;
...
int swm::ei_buffer_to_scheduler(const char *buf, const int *index, std::vector<SwmScheduler> &array) {
  int term_size = 0
  int term_type = 0;
  ...
  if (term_type != ERL_LIST_EXT) {
      std::cerr << "Could not parse term: not a scheduler list at position " << index << std::endl;
      return -1;
  }
  int list_size = 0;
