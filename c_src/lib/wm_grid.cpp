...
SwmGrid::SwmGrid(const char* buf, int* index) {
  if (!buf) {
    std::cerr << "Cannot convert ei buffer into SwmGrid: null" << std::endl;
    return;
  }
  int term_size = 0;
...
int swm::ei_buffer_to_grid(const char *buf, const int *index, std::vector<SwmGrid> &array) {
  int term_size = 0
  int term_type = 0;
  ...
  if (term_type != ERL_LIST_EXT) {
      std::cerr << "Could not parse term: not a grid list at position " << index << std::endl;
      return -1;
  }
  int list_size = 0;
