...
-export([suite/0, all/0, init_per_suite/1, end_per_suite/1, init_per_testcase/2,
         end_per_testcase/2]).
-export([entrypoint_default_no_tini/1, entrypoint_override/1, finalize_prefers_container_env/1,
         run_steps_podman/1, podman_sock_override/1, cdi_missing_msg/1, require_crun_default/1,
         cdi_unavailable_without_specs/1, cdi_available_with_nvidia_json/1,
         communicate_steps_podman/1]).
...
cdi_available_with_nvidia_json(_Config) ->
    ...
    after
        _ = file:delete(filename:join(Dir, "nvidia.com-gpu.json")),
        _ = file:del_dir(Dir)
    end.
